...
- **Play**: `docs/replay.js` and `replay.css`. The watched car drives in colour, every other entry of the board greyed out and lined up on its ranked lap's start, the TA car always picked out; hover names a car, a click or the list switches to it. Full screen (`F`), 0.25x to 8x. `?watch=<alias or team>` opens a run directly, which is what Copy link shares. Track images are `docs/assets/maps/`, made by `python3 tools/make_maps.py <dir with the map .yaml/.png>`; a recording whose map is not in `maps.json` is refused, so add the map before the lab opens.
