...
Every timed lab gets a tab. A row is one driver's best lap in the grading simulator, shown under a racing alias: their best full-score submission graded before the deadline, among their first five graded submissions. The TA solution sits in the table as an unranked reference row. The top five earn extra credit.

## How it works

- **What counts.** A lab is on the board when its grader config (`classroom50/ese-6150/autograders/<lab>/config.yaml`) has a `leaderboard:` block: which test's detail string carries the metric, the regex that reads it, the direction, the podium size. The same file's `submissions.cap` is the number of graded submissions a repo gets.
- **Rebuilds.** The workflow in this repo runs every five minutes (and on demand: Actions, "Build the board", "Run workflow"). It reads `assignments.json` and the grader configs from the private class repo, lists each student repo's `submit/*` releases, downloads new `result.json` files, and rewrites `data/<lab>.json` and `data/index.json`. Changed data is committed to `main`, which GitHub Pages serves. The minutes are free because this repo is public.
...
- **The cap.** Once a repo has five graded submissions the builder runs the class's own `lab-access.sh lock <lab> --user <login>` (falling back to the collaborator API) and the repo becomes read-only. The grader itself refuses a sixth push that slips in before the lock, with the commit status "submission cap reached: not graded, grade unchanged" and no release.
...
- **Refund an attempt** after a grading-infrastructure failure: `GH_TOKEN=$(gh auth token) python3 -m builder refund lab-3-wall-following <username> submit/<tag>` marks that submission refunded and unlocks the repo. Commit `data/lab-3-wall-following.json` and push; the next build re-posts the student's note.
...
Public: aliases, lap times, submission timestamps, attempt counts, lock state, keyed owner fingerprints. Never usernames, repo names, or scores. The workflow log is public too, so the builder redacts every username and repo name it encounters before printing.
